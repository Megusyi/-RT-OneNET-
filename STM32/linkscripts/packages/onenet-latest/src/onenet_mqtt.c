...
static void mqtt_usr_callback(MQTTClient *c, MessageData *msg_data)
{
    size_t res_len = 0;
    uint8_t *response_buf = RT_NULL;
    char topicname[80] = { 0 };
    ...
    LOG_I("Topic: %.*s", msg_data->topicName->lenstring.len, msg_data->topicName->lenstring.data);
    LOG_I("Message length: %d", msg_data->message->payloadlen);
    LOG_I("Message content: %.*s", msg_data->message->payloadlen, (char*)msg_data->message->payload);

    if (onenet_mqtt.cmd_rsp_cb != RT_NULL)
    {
        LOG_I("Calling cmd_rsp_cb...");
        onenet_mqtt.cmd_rsp_cb((uint8_t *) msg_data->message->payload, msg_data->message->payloadlen, &response_buf, &res_len);

        if (response_buf != RT_NULL && res_len != 0)
        {
            /* 构造正确的响应 topic: $sys/{pid}/{devid}/thing/property/set_reply */
            rt_snprintf(topicname, sizeof(topicname), "$sys/%s/%s/thing/property/set_reply",
                        onenet_info.pro_id, onenet_info.device_id);
            LOG_I("Response topic: %s", topicname);
            LOG_I("Response length: %d", res_len);
            onenet_mqtt_publish(topicname, response_buf, res_len);
            ONENET_FREE(response_buf);
        }
        else
        {
            LOG_I("No response data");
        }
    }
    else
    {
        LOG_E("cmd_rsp_cb is NULL! Please call onenet_set_cmd_rsp_cb first!");
    }
}
...
#else
    strncpy(dev_id, ONENET_INFO_DEVID, sizeof(dev_id) - 1);
    dev_id[sizeof(dev_id) - 1] = '\0';
    strncpy(auth_info, ONENET_INFO_AUTH, sizeof(auth_info) - 1);
    auth_info[sizeof(auth_info) - 1] = '\0';
#endif

    strncpy(onenet_info.device_id, dev_id, sizeof(onenet_info.device_id) - 1);
    onenet_info.device_id[sizeof(onenet_info.device_id) - 1] = '\0';
    strncpy(onenet_info.api_key, api_key, sizeof(onenet_info.api_key) - 1);
    onenet_info.api_key[sizeof(onenet_info.api_key) - 1] = '\0';
    strncpy(onenet_info.pro_id, ONENET_INFO_PROID, sizeof(onenet_info.pro_id) - 1);
    onenet_info.pro_id[sizeof(onenet_info.pro_id) - 1] = '\0';
    strncpy(onenet_info.auth_info, auth_info, sizeof(onenet_info.auth_info) - 1);
    onenet_info.auth_info[sizeof(onenet_info.auth_info) - 1] = '\0';
    strncpy(onenet_info.server_uri, ONENET_SERVER_URL, sizeof(onenet_info.server_uri) - 1);
    onenet_info.server_uri[sizeof(onenet_info.server_uri) - 1] = '\0';
 ...
 *         -1 : subscribe fail
 */
int onenet_mqtt_subscribe(const char *topic, enum QoS qos)
{
    return paho_mqtt_subscribe(&mq_client, qos, topic, NULL);
}